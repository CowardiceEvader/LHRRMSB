...
`VISION_TOE` 的在线判定依赖 `detect_task` 的时间窗机制，只有收到**完整且校验通过**的 `CMD_NAV_DATA` 或 `CMD_HEARTBEAT` 才会调用 `detect_hook(VISION_TOE)` 刷新时间戳。

- 刚收到有效帧后的约 `80 ms` 内：处于“刚恢复、未稳定”阶段，`toe_is_error(VISION_TOE)` 仍为错误态。
- 超过约 `80 ms` 且未超过 `120 ms` 超时窗口：会转为在线（非错误态）。
- 若之后不再收到新有效帧：到约 `120 ms` 超时后又会掉线。

因此，单帧不能“立刻打开在线”；它最多只会在时序上短暂进入在线窗口。工程上应持续周期发送有效帧（优先持续发送 `CMD_NAV_DATA`）。

### 时间线示意图（0ms / 80ms / 120ms）

以下示意假设 `t=0` 时刻刚收到一帧**有效帧**（`NAV_DATA` 或 `HEARTBEAT`）：

```text
t=0ms                    t=80ms                           t=120ms
|------------------------|--------------------------------|-------------------->
收到有效帧并refresh        恢复稳定窗口结束                  若一直无新有效帧，判定掉线
VISION_TOE时间戳          （开始可视为稳定在线）             （failsafe清零）

状态理解：
- 0~80ms：恢复/去抖阶段（toe_is_error 仍可能为 1）
- 80~120ms：稳定在线窗口（前提是期间未超时）
- >120ms 且无新有效帧：离线
```

若在 `120ms` 之前收到下一帧有效帧，时间轴会“重新从 0ms 计时”，链路继续保持在线。
